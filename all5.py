...
def extract_nodes_elements(results):
    ...
    for box in results.obb:
        ...
        if name in ["pin", "roller", "fixed"]:
            ...
            loads.append({'name': name, 'pos': center, 'value': 1000})  # 仮荷重値
...
def plot_stress(nodes, elements, displacements):
    ...
    for el in elements:
        ...
        idx1 = next(i for i,n in enumerate(nodes) if np.allclose(n['pos'], n1))
        idx2 = next(i for i,n in enumerate(nodes) if np.allclose(n['pos'], n2))
