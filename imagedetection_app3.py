...
def find_scale_line(lines):
    """
    長さ10cm（=0.1m）の基準物ラインを探す。
    だいたい一番長い短い線として探す（30px以上100px以下など）。
    横か縦の線を想定。
    """
    candidate_lines = []
    if lines is None:
    ...
    for line in lines:
        ...
        # 横か縦に近い線
        if (abs(y2 - y1) < 5 or abs(x2 - x1) < 5) and (30 <= length <= 150):
...
if uploaded_file is not None:
    ...
    scale_line = find_scale_line(lines)
    ...
    if scale_line is None:
        ...
        # ここから梁や荷重の検出に移る（前に作ったやつをスケール対応で）
        ...
        if beam_line is None:
            ...
            # 応力図作成
            x_vals = np.linspace(0, beam_length_m, int(beam_length_m * 100))  # 100分割
            ...
            for (cx, cy, length_px) in loads:
                ...
                load_val = length_px * 5  # 1マス＝5N相当で適当にスケール
