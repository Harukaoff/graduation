...
st.set_page_config(layout="wide", page_title="構造図 清書 + 可視化 (fixed-v4)")
...
def compute_placement_for_element(pts_raw, box_xywhr, cls_name, beam_lines):
    ...
    if cls_name in ("pin", "roller"):
        ...
        # --- TEMPLATE DEFAULT: pin/roller "tip is UP" ---
        # Model angle_deg is direction we want the tip to point to.
        # Template's tip points UP (90°), so rotate template by (angle - 90)
        angle_for_template = (angle_deg - 90.0)
        # normalize
        angle_for_template = ((angle_for_template + 180.0) % 360.0) - 180.0

        result.update({"center": centre_shifted, "angle": angle_for_template, "scale": avg_short})
        return result
    ...
    if cls_name in ("fixed", "fix", "kotei"):
        ...
        if np.linalg.norm(v) < 1e-6:
            v = np.array([1.0, 0.0])
        angle_deg = math.degrees(math.atan2(v[1], v[0]))
        # NOTE: Template default: right side is straight. We want straight side to align with beam direction.
        # So no +90 correction here (we removed earlier +90).
        angle_for_template = angle_deg
        angle_for_template = ((angle_for_template + 180.0) % 360.0) - 180.0
...
# ---------------------------
def main():
    st.title("🏗️ 構造図 清書アプリ（fixed-v4）")
    st.write("・ピン/ローラーのデフォ向き(top) に合わせた完全版")
    ...
    for elem, place in placements:
        ...
        tpl = TEMPL.get(name)
        center = place["center"]
        angle = float(place["angle"])  # angle already computed to be template-rotation angle
        ...
        if name in ("pin", "roller", "fixed", "udl"):
            angle = (angle - 90) % 360
        ...
        if "beam" in name:
            ...
            # pin/roller template: tip is UP in file. We computed angle accordingly in compute_placement.
            # fixed template: straight side is RIGHT in file. angle computed accordingly.
            tpl_rot = rotate_image_keep_alpha(tpl_scaled, angle)

            # For correct placement, compute overlay center so that template center maps to desired center.
            cleaned = overlay_rgba(cleaned, tpl_rot, center)
