...
# ===============================
def run_app():
    st.title("🏗️ 構造図 清書アプリ（梁角度・スケール補正版）")
            ...
            with st.spinner("検出・清書中..."):
                ...
                if hasattr(results, "obb") and results.obb is not None:
                    for box in results.obb:
                        ...
                        # 30度単位に丸める
                        angle_deg = round(angle_deg / 30) * 30

                        # 梁のスケール補正（縦梁ゴマ防止）
                        if "beam" in name:
                            if w < h:
                                angle_deg += 90
                                scale = h / templates[name].shape[0]
                            else:
                                scale = w / templates[name].shape[1]
                            beams.append((x, y, w, h, angle_deg))
                        else:
                            scale = max(w / templates[name].shape[1], 0.2)
