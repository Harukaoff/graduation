...
def main():
    ...
    for elem, place in placements:
        ...
        # snap to 15° for those that use angle
        # But pin/roller will ignore rotation per user request.
        if tpl is None:
        ...
        if "beam" in name:
            ...
            # USER REQUEST: place template as-is, ignore detected angle.
            tpl_scaled = scale_image(tpl, factor)
            # center the scaled template on detected center without rotation
            cleaned = overlay_rgba(cleaned, tpl_scaled, center)

        elif name == "fixed":
            # keep rotation for fixed
            tpl_scaled = scale_image(tpl, factor)
            angle_use = round(angle / 15.0) * 15.0
            tpl_rot = rotate_image_keep_alpha(tpl_scaled, angle_use)
            cleaned = overlay_rgba(cleaned, tpl_rot, center)
