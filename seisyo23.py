...
st.set_page_config(layout="wide", page_title="構造図 清書 + 可視化 (fixed-v3)")
...
def overlay_rgba(base: np.ndarray, overlay: np.ndarray, center: Tuple[float,float]):
    ...
    for c in range(3):
        base[Y1:Y2, X1:X2, c] = (1-alpha[...,0]) * base[Y1:Y2,X1:X2,c] + alpha[...,0] * crop[...,c]
    return base
...
def compute_placement_for_element(pts_raw, box_xywhr, cls_name, beam_lines):
    ...
    if "beam" in cls_name:
        ...
        centerline_mid = (m1+m2)/2
        result.update({"center":centerline_mid,"angle":angle_deg,"scale":long_len})
        return result
    ...
    if cls_name in ("pin","roller"):
        if nearest_beam is not None:
            beam_mid = (nearest_beam["a"] + nearest_beam["b"])/2
            ...
            avg_short = (edges[short_idxs[0]]["len"] + edges[short_idxs[1]]["len"])/2
            shift_dir = desired_vec / (np.linalg.norm(desired_vec)+1e-12)
            result.update({"center":center + shift_dir*(avg_short*0.02),
                           "angle":angle_deg,"scale":avg_short})
            return result
        else:
            _,_,_,_,ang = box_xywhr
            angle_deg = -math.degrees(ang)
    ...
    if cls_name in ("load","kajyu"):
        ...
        long_len = max(edges[long_idxs[0]]["len"], edges[long_idxs[1]]["len"])
        result.update({"center":seg_mid,"angle":angle_deg,"scale":long_len})
        return result
...
# ---------------------------
def main():
    st.title("🏗️ 構造図 清書アプリ（fixed-v3）")
    st.write("・検出に基づきつつ、支点/荷重のズレを完全修正したバージョン")
    ...
    for elem, place in placements:
        ...
        center = place["center"]
        angle = round(float(place["angle"])/15)*15
        raw_scale = float(place["scale"])
        ...
        if "beam" in name:
            ...
            cx0,cy0 = w0/2, h0/2
            M = cv2.getRotationMatrix2D((cx0,cy0),angle,1)
            tip_h = np.array([tip_px[0],tip_px[1],1])
            tip_rot = M.dot(tip_h)

            tpl_rot = rotate_image_keep_alpha(tpl_scaled, angle)
            tpl_rot_h, tpl_rot_w = tpl_rot.shape[:2]

            tip_rot[0] += (tpl_rot_w/2 - cx0)
            tip_rot[1] += (tpl_rot_h/2 - cy0)

            shift_x = center[0] - tip_rot[0]
            shift_y = center[1] - tip_rot[1]
            overlay_center = (shift_x + tpl_rot_w/2, shift_y + tpl_rot_h/2)
