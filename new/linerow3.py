...
st.title("構造図の清書（水平梁を一本化＋三角形検出＋近似精度UP）")
...
if uploaded_file is not None:
    ...
    c_value = st.sidebar.slider("adaptiveThreshold C", 0, 20, 6)
    ...
    def fit_and_draw_horizontal_line_fitline(blank, group, color, thickness, index, line_index_map):
        # グループ内の全端点をリストアップし、fitLineで最小二乗直線近似
        pts = []
        raw_indices = []
        for line in group:
            x1, y1, x2, y2 = line[0]
